...
#if IWLS_TLS_MODE == IWLS_TLS_BUNDLE
...
#elif IWLS_TLS_MODE == IWLS_TLS_PINNED
    static_assert(sizeof(IWLS_ROOT_CA) > 1,
                  "IWLS_TLS_PINNED needs a certificate in IWLS_ROOT_CA; see tools/fetch_ca.sh");
    client.setCACert(IWLS_ROOT_CA);
#else
    client.setInsecure();
#endif
}
...
bool httpGet(const char* url, String& body, bool& tlsError) {
    ...
    if (code != HTTP_CODE_OK) {
        ...
        char reason[128] = {0};
        if (client.lastError(reason, sizeof(reason)) != 0) {
            Serial.printf("[tls] %s\n", reason);
            tlsError = true;
        }
        http.end();
...
bool fetchSeries(int64_t now, const char* series, int hoursBefore, int hoursAfter,
                 const char* resolution, String& body, bool& tlsError) {
    char from[32], to[32];
    ...
    return httpGet(url, body, tlsError);
}
...
const char* fetchStatusMessage(FetchStatus status) {
    switch (status) {
        ...
        case FetchStatus::kTlsFailed:
            return "COULD NOT VERIFY THE TIDE SERVICE CERTIFICATE";
...
FetchStatus iwlsRefresh(TideData& data) {
    ...
    bool tlsError = false;
    {
        ...
        if (!fetchSeries(now, "wlp-hilo", HILO_HOURS_BEFORE, HILO_HOURS_AFTER, nullptr, body,
                         tlsError)) {
            status = tlsError ? FetchStatus::kTlsFailed : FetchStatus::kHttpFailed;
    ...
    if (status == FetchStatus::kOk) {
        String body;
        if (!fetchSeries(now, "wlp", CURVE_HOURS_BEFORE, CURVE_HOURS_AFTER, CURVE_RESOLUTION, body,
                         tlsError)) {
            status = tlsError ? FetchStatus::kTlsFailed : FetchStatus::kHttpFailed;
